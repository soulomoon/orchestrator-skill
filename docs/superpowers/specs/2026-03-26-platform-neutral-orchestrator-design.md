...
For verification purposes, active surfaces are:

- the live skill folders under `skills/`
- the repository README
- scaffold assets shipped by those skills
- reference documents loaded by those skills
- live target-repo contract files reachable from `orchestrator/state.json`
...
Role files are generated by the scaffold skill and are part of the required
contract for supported repositories. Missing role files should be treated as an
invalid scaffold state, not as a scenario that needs compatibility design.
